...
def avaliacao_quantidade_de_pecas(board: Board, cor):
    return 100*board.num_pieces(cor)/ (board.num_pieces(cor)+board.num_pieces(inverte_cor(cor)))


def avaliacao_menos_espacos_vazios(board: Board, cor):
    return 100*-board.num_pieces(Board.EMPTY) / 64


def avaliacao_diferenca_pecas(board: Board, cor):
    return 100* board.num_pieces(cor) - board.num_pieces(inverte_cor(cor)) / (board.num_pieces(cor) + board.num_pieces(inverte_cor(cor)) )
