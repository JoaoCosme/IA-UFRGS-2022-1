...
def player_max(tabuleiro, color, alpha, beta, depth):  # Função que representa o jogador MAX
    ...
    if len(legal_moves) == 0:
        # Se jogador atual não possui jogadas válidas, devolve movimeto vazio
        legal_moves = [NULL_MOVE]
    ...
    for move_prox in legal_moves:
        ...
        temp_val, temp_move = player_min(state, tabuleiro.opponent(
            color), alpha, beta, depth + 1)  # Define se deve atualizar ou não o novo movimento
...
def player_min(tabuleiro, color, alpha, beta, depth):  # Função que representa o jogador MIN
    ...
    for new_move in legal_moves:
        ...
        temp_val, temp_move = player_max(state, tabuleiro.opponent(
            color), alpha, beta, depth + 1)  # Define se deve atualizar ou não o novo movimento
...
def make_move(the_board: Board, color):
    ...
    # começa o MINMAX chamando a função MAX para jogar
    val, move = player_max(the_board, color, VAL_MIN, VAL_MAX, DEPTH_START)
